...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################################
        # Replace "pass" statement with your code
        feats = self.encoder(x)  # hidden features
        mu = self.mu_layer(feats)
        logvar = self.logvar_layer(feats)
...
class CVAE(nn.Module):
    def __init__(self, input_size, num_classes=10, latent_size=15):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(input_size+num_classes, self.hidden_dim),  # H*W + C
            nn.ReLU(),
            nn.Linear(self.hidden_dim, self.hidden_dim),
            nn.ReLU(),
            nn.Linear(self.hidden_dim, self.hidden_dim),
        ...
        # Replace "pass" statement with your code
        self.decoder = nn.Sequential(
            nn.Linear(latent_size+num_classes, self.hidden_dim),  # Z + C
            nn.ReLU(),
            nn.Linear(self.hidden_dim, self.hidden_dim),
            nn.ReLU(),
            nn.Linear(self.hidden_dim, self.hidden_dim),
    ...
    def forward(self, x, c):
        ...
        # Replace "pass" statement with your code
        x = self.flatten(x)
        feats = self.encoder(torch.cat((x, c), dim=-1))  # hidden features
        mu = self.mu_layer(feats)
        logvar = self.logvar_layer(feats)
        z = reparametrize(mu, logvar)
...
def reparametrize(mu, logvar):
    ...
    ################################################################################################
    # Replace "pass" statement with your code
    epsilon = torch.randn(mu.shape, device=mu.device)  # normal distribution with mean 0 and variance 1
    z = logvar.exp().sqrt() * epsilon + mu  # turn logvar to sigma(standard deviation)
    ################################################################################################
    #                              END OF YOUR CODE                                                #
    ################################################################################################
    return z
...
def loss_function(x_hat, x, mu, logvar):
    ...
    recons_loss = F.binary_cross_entropy(x_hat, x, reduction='sum') / N  # reconstruction loss
    kl_div = (-1./2 * torch.sum((1 + logvar - mu**2 - logvar.exp()), dim=1)).mean()  # KL divergence
