...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  fmap_h = 1 + (H + 2 * 2 - kernel_size_1) / 1
  fmap_w = 1 + (W + 2 * 2 - kernel_size_1) / 1
  ...
  fmap_h = 1 + (fmap_h + 2 * 1 - kernel_size_2) / 1
  fmap_w = 1 + (fmap_w + 2 * 1 - kernel_size_2) / 1
  
  fc_w = nn.init.kaiming_normal_(torch.empty((num_classes, int(fmap_h*fmap_w*channel_2)), dtype=dtype, device=device))
...
class ThreeLayerConvNet(nn.Module):
  def __init__(self, in_channel, channel_1, channel_2, num_classes):
    ...
    self.conv1 = nn.Conv2d(in_channel, channel_1, kernel_size_1, padding=2)
    self.conv2 = nn.Conv2d(channel_1, channel_2, kernel_size_2, padding=1)
    fmap_h = 1 + (H + 2 * 2 - kernel_size_1) / 1  # after conv1
    fmap_w = 1 + (W + 2 * 2 - kernel_size_1) / 1
    fmap_h = 1 + (fmap_h + 2 * 1 - kernel_size_2) / 1  # after conv2
    fmap_w = 1 + (fmap_w + 2 * 1 - kernel_size_2) / 1
    self.fc = nn.Linear(int(channel_2*fmap_h*fmap_w), num_classes)
...
def initialize_three_layer_conv_part4():
  ...
  ####################################################################################
  # Replace "pass" statement with your code
  fmap_h = 1 + (H + 2 * pad_size_1 - kernel_size_1) / 1  # after conv1
  fmap_w = 1 + (W + 2 * pad_size_1 - kernel_size_1) / 1
  fmap_h = 1 + (fmap_h + 2 * pad_size_2 - kernel_size_2) / 1  # after conv2
  fmap_w = 1 + (fmap_w + 2 * pad_size_2 - kernel_size_2) / 1
  ...
  model = nn.Sequential(OrderedDict([
    ...
    ('relu2', nn.ReLU()),
    ('flatten', Flatten()),
    ('fc', nn.Linear(int(fmap_h*fmap_w*channel_2), num_classes)),
    ]))
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.net = nn.Sequential(
      nn.BatchNorm2d(Cin),  # Spatial Batchnorm
      nn.ReLU(),
      nn.Conv2d(Cin, Cout, 3, padding=1, stride=stride),
      nn.BatchNorm2d(Cout),  # Spatial Batchnorm
      nn.ReLU(),
...
class ResidualBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    if Cin == Cout and not downsample:
      ...
      self.shortcut = nn.Conv2d(Cin, Cout, 1, stride=stride)
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    stem_cout = stage_args[0][0]
    blocks = [ResNetStem(Cin, stem_cout)]
    for spec in stage_args:
      blocks.append(ResNetStage(*spec, block=block))
    self.cnn = nn.Sequential(*blocks)
    
    # global average pooling, turn shape(N, C', H', W') to (N, C', 1, 1); become (N, stage_args[-1][1]) after flatten
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.plainblock = nn.Sequential(
      ...
      nn.BatchNorm2d(Cout//4),  # Spatial Batchnorm
      nn.ReLU(),
