...
def compute_saliency_maps(X, y, model):
  ...
  loss.backward()
  saliency = torch.abs(X.grad.data).max(dim=1).values  # X.grad.data == X.grad ?
  ##############################################################################
  #               END OF YOUR CODE                                             #
  ##############################################################################
  return saliency
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  for i in range(max_iter):
    ...
    if(max_score_idx == target_y):
      print('Successful, the model is fooled!!')
      break
    
    target_score.backward()
    dX = learning_rate * X_adv.grad / ((X_adv.grad**2).sum().sqrt() + 1e-5)  # normalize the gradient
    
    with torch.no_grad():
      X_adv += dX  # increase the score with respect to target_y
      X_adv.grad.zero_()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    r = l2_reg * (img**2).sum()  # λ∥I∥_2^2
    target_score -= r
    target_score.backward()
      
    with torch.no_grad():
      img += learning_rate * img.grad / ((img.grad**2).sum().sqrt() + 1e-5)
      img.grad.zero_()
    ########################################################################
    #                             END OF YOUR CODE                         #
    ########################################################################
