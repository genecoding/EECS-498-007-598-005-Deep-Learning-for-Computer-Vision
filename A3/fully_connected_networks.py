...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    W2 = self.params['W2']
    b2 = self.params['b2']
    l1_out, l1_cache = Linear_ReLU.forward(X, W1, b1)  # layer1 out, after nonlinearity
    scores, l2_cache = Linear.forward(l1_out, W2, b2)  # layer2 out, without nonlinearity
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    ...
    # Replace "pass" statement with your code
    reg = self.reg
    loss, d_scores = softmax_loss(scores, y)  # loss, d_scores already /N
    loss += reg * (torch.sum(W1 * W1) + torch.sum(W2 * W2))
    
    l2_dx, l2_dw, l2_db = Linear.backward(d_scores, l2_cache)
    grads['W2'] = l2_dw + 2 * reg * W2
    grads['b2'] = l2_db
    l1_dx, l1_dw, l1_db = Linear_ReLU.backward(l2_dx, l1_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    ############################################################################
    # Replace "pass" statement with your code
    weight_dims = [input_dim] + hidden_dims + [num_classes]
    for n in range(self.num_layers):
      # W1(input_dim * hidden_dims[0]), W2(hidden_dims[0] * hidden_dims[1]), ..., Wn(hidden_dims[-1] * num_classes)
      self.params['W' + str(n+1)] = weight_scale * torch.randn(weight_dims[n], weight_dims[n+1], dtype=dtype, device=device)
      # b1(hidden_dims[0]), b2(hidden_dims[1]), ..., bn(num_classes)
      self.params['b' + str(n+1)] = torch.zeros(weight_dims[n+1], dtype=dtype, device=device)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for n in range(self.num_layers-1):
      w_num = 'W' + str(n+1)
      b_num = 'b' + str(n+1)
      layer_out, layer_cache = Linear_ReLU.forward(layer_out, self.params[w_num], self.params[b_num])  # layer out, after nonlinearity
      layer_caches.append(layer_cache)
      if self.use_dropout:
        layer_out, layer_cache = Dropout.forward(layer_out, self.dropout_param)
        layer_caches.append(layer_cache)
    
    # compute the last layer(output layer) with Wn
    w_num = 'W' + str(self.num_layers)
    b_num = 'b' + str(self.num_layers)
    scores, last_cache = Linear.forward(layer_out, self.params[w_num], self.params[b_num])  # the last layer out, without nonlinearity
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    w_num = 'W' + str(self.num_layers)
    b_num = 'b' + str(self.num_layers)
    loss += 0.5 * reg * torch.sum(self.params[w_num] * self.params[w_num])  # include a factor of 0.5!
    dx, dw, db = Linear.backward(d_scores, last_cache)
    grads[w_num] = dw + reg * self.params[w_num]  # don't need to multiply 2
    grads[b_num] = db
    ...
    for n in range(self.num_layers-1, 0, -1):
      ...
      loss += 0.5 * reg * torch.sum(self.params[w_num] * self.params[w_num])  # include a factor of 0.5!
      if self.use_dropout:
        layer_cache = layer_caches.pop()
        dx = Dropout.backward(dx, layer_cache)
      layer_cache = layer_caches.pop()
...
def adam(w, dw, config=None):
  ...
  t += 1  # t is your iteration counter going from 1 to infinity
...
class Dropout(object):
  ...
  def forward(x, dropout_param):
    ...
    if mode == 'train':
      ...
      ###########################################################################
      # Replace "pass" statement with your code
      # p for 'drop' probability, then the 'keep' probability is 1-p. 
