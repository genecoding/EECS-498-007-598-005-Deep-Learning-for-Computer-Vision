...
class ProposalModule(nn.Module):
  ...
  def forward(self, features, pos_anchor_coord=None, \
    ...
    outputs = self.pred_layer(features)  # (B, A*6, 7, 7)
    outputs = outputs.reshape(B, A, -1, H, W)  # (B, A, 6, H', W')
    ...
    if mode == 'train':
      conf_scores = torch.cat((self._extract_anchor_data(conf_scores, pos_anchor_idx), self._extract_anchor_data(conf_scores, neg_anchor_idx)))
      offsets = self._extract_anchor_data(offsets, pos_anchor_idx)
      # reshape anchors and offsets to the same shape to use GenerateProposal()
      proposals = GenerateProposal(pos_anchor_coord.reshape(1, -1, 1, 1, 4), offsets.reshape(1, -1, 1, 1, 4), method='FasterRCNN').reshape(-1, 4)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    if mode == 'train':
...
class RPN(nn.Module):
  ...
  def forward(self, images, bboxes, output_mode='loss'):
    ...
    pos_anchor_idx, neg_anchor_idx, _, GT_offsets, GT_class, pos_anchor_coord, _ = \
      ReferenceOnActivatedAnchors(anchors, bboxes, grids, iou, method='FasterRCNN')
    
    # iv) Compute conf_scores, offsets, proposals through the region proposal module
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7, mode='RPN'):
    ...
    # iii) Compute conf_scores, offsets through the region proposal module
    conf_scores, offsets = self.prop_module.forward(features)  # (B, A, 2, H', W'), (B, A, 4, H', W')
    conf_probs = torch.sigmoid(conf_scores.permute(0, 1, 3, 4, 2)).reshape(B, -1, 2)[:, :, 0]  # pick up probability of object part
    conf_p_gtthres_idx = conf_probs > thresh  # (B, -1)
    
    # iv) proposal generation
    proposals = GenerateProposal(anchors, offsets.permute(0, 1, 3, 4, 2), method='FasterRCNN')  # (B, A, H, W, 4)
    proposals = proposals.reshape(B, -1, 4)
    
    # v) compute final_proposals & final_conf_probs
    final_proposals, final_conf_probs = [], []
    with torch.no_grad():
      for b in range(B):
        filtered_prop = proposals[b][conf_p_gtthres_idx[b]]
        filtered_conf_p = conf_probs[b][conf_p_gtthres_idx[b]]
        nms_idx = torchvision.ops.nms(filtered_prop, filtered_conf_p, nms_thresh)
        final_proposals.append(filtered_prop[nms_idx])
        final_conf_probs.append(filtered_conf_p[nms_idx].unsqueeze(1))  # of shape (P_i, 1)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    if mode == 'RPN':
...
class TwoStageDetector(nn.Module):
  def __init__(self, in_dim=1280, hidden_dim=256, num_classes=20, \
    ...
    self.meanpool = nn.AvgPool2d((roi_output_h, roi_output_w))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
  ...
  def forward(self, images, bboxes):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    # i) RPN, including image feature extraction, grid/anchor/proposal generation, activated and negative anchors determination.
    rpn_loss, _, proposals, features, GT_class, pos_anchor_idx, anc_per_img = \
      self.rpn.forward(images, bboxes, output_mode='all')

    # ii) Perform RoI Align on proposals and meanpool the feature in the spatial dimension.
    img_no = (pos_anchor_idx // anc_per_img).reshape(-1, 1)
    img_no_props = torch.cat((img_no, proposals), dim=1)  # proposals with image No.
    roi_features = torchvision.ops.roi_align(features, img_no_props, output_size=(self.roi_output_h, self.roi_output_w))  # (# of proposals, C=1280, 2, 2)
    roi_features = self.meanpool(roi_features)  # (# of proposals, C, 1, 1)
    
    # iii) Pass the RoI feature through the region classification layer which gives the class probilities.
    cls_prob = self.cls_layer(roi_features.flatten(1))
    
    # iv) Compute class_prob through the prediction network and compute the cross entropy loss (cls_loss)
    cls_loss = nn.functional.cross_entropy(cls_prob, GT_class)
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    # i) RPN, including image feature extraction, final_proposals/final_conf_probs generation
    final_proposals, final_conf_probs, features = \
      self.rpn.inference(images, thresh=thresh, nms_thresh=nms_thresh, mode='FasterRCNN')
      
    # ii) Perform RoI Align on proposals and meanpool the feature in the spatial dimension.
    roi_features = torchvision.ops.roi_align(features, final_proposals, output_size=(self.roi_output_h, self.roi_output_w))  # (# of proposals, C=1280, 2, 2)
    roi_features = self.meanpool(roi_features)  # (# of proposals, C, 1, 1)
    
    # iii) Pass the RoI feature through the region classification layer which gives the class probilities.
    cls_prob = self.cls_layer(roi_features.flatten(1))
    pred_cls = cls_prob.max(dim=1).indices.unsqueeze(1)  # of shape (P_i, 1)
    props_per_img = [p.shape[0] for p in final_proposals]
    final_class = list(torch.split(pred_cls, props_per_img))  # split() returns a tuple
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
