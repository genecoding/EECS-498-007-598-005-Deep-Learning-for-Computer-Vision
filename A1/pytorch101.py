...
def multiples_of_ten(start, stop):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  if start % 10 == 0:  # start is a multiple of ten
    pass
  else:
    start += 10 - (start % 10)  # add 1~9 to make start the first multiple of ten in the range
      
  if start > stop:  # there is no multiple of ten in the range
...
def reshape_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  y = x.view(2, 3, 4).permute(1, 2, 0).transpose(1, 2).reshape(3, 8)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def zero_row_min(x):
  ...
  # Replace "pass" statement with your code
  y = torch.clone(x)
  idx0 = list(range(x.shape[0]))
  idx1 = torch.argmin(x, dim=1)
  y[idx0, idx1] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  # Replace "pass" statement with your code
  if(use_loop):
    z = torch.stack([torch.mm(x[b], y[b]) for b in range(x.shape[0])], dim=0)
  else:
    z = torch.bmm(x, y)
  #############################################################################
  #                            END OF YOUR CODE                               #
...
def normalize_columns(x):
  ...
  # Replace "pass" statement with your code
  n_row = x.shape[0]
  mu_j = torch.sum(x, dim=0) / n_row
  sigma_j = torch.sqrt(1/(n_row-1) * torch.sum(((x-mu_j)**2), dim=0))
  y = (x-mu_j) / sigma_j
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
