...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    for j in range(num_test):
      dists[i][j] = torch.sum((train_flat[i] - test_flat[j]) ** 2)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  test_flat = x_test.flatten(start_dim=1)
  for i in range(num_train):
    dists[i] = torch.sum((train_flat[i].view(1, -1) - test_flat) ** 2, dim=1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  #(a-b)^2 = a^2 - 2ab + b^2
  dists = torch.sum(train_flat**2, dim=-1).view(-1, 1) + torch.sum(test_flat**2, dim=-1) - 2*train_flat.mm(test_flat.t())
  
  # slower than 1 loop
  # dists = torch.sum((train_flat.view(num_train, 1, -1) - test_flat) ** 2, dim=-1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  short_dists = torch.topk(dists, k=k, dim=0, largest=False)
  cls_of_k = y_train[short_dists.indices]
  
  for i in range(num_test):
    out, count = torch.unique(cls_of_k[:, i], dim=0, return_counts=True)
    y_pred[i] = out[torch.argmax(count)]  # find the class of majority of the data(one col)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      x_train = list(x_train_folds)  # after torch.chunk x_train_folds becomes tuple, turn it to list
      y_train = list(y_train_folds)
      x_valid = x_train_folds[i]
      y_valid = y_train_folds[i]
      del x_train[i]
      del y_train[i]
      x_train = torch.cat(x_train, dim=0)  # combine & turn it to tensor
      y_train = torch.cat(y_train, dim=0)
      
      k_classifier = KnnClassifier(x_train, y_train)
      acc = k_classifier.check_accuracy(x_valid, y_valid, k=k)
      acc_for_fold.append(acc)

    k_to_accuracies[k] = acc_for_fold
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
...
def knn_get_best_k(k_to_accuracies):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  best_k = max(k_to_accuracies.keys(), key=lambda k: max(k_to_accuracies[k]))
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  return best_k
