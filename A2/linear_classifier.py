...
def svm_loss_vectorized(W, X, y, reg):
  ...
  num_train = X.shape[0]
  scores = X.mm(W)
  idx0 = torch.arange(num_train)
  correct_class_scores = scores[idx0, y]
  margins = torch.maximum(torch.tensor(0), scores - correct_class_scores.view(-1, 1) + 1)  # delta = 1
  margins[idx0, y] = 0  # for correct classes
  ...
  #############################################################################
  # Replace "pass" statement with your code
  mask = (margins > 0).double()
  num_gt_margin = torch.sum(mask, dim=1)  # num of > margin per row
  mask[idx0, y] -= num_gt_margin
  dW = X.t().mm(mask)
...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  # a = torch.tensor([10**i for i in range(-9, 10)])
  # b = torch.tensor([i for i in range(1, 10)])
  # c = (a.view(-1, 1) * b).flatten()
  # learning_rates = c.tolist()
  # regularization_strengths = c.tolist()
  
  learning_rates = [9.6e-1, 9.6e-2, 9.6e-3, 9.6e-4, 9.6e-5]
  regularization_strengths = [5.5e-3, 5.5e-4, 5.5e-5, 5.5e-6, 5.5e-7]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  pred_train = cls.predict(data_dict['X_train'])
  comp = torch.eq(pred_train, data_dict['y_train'])
  train_acc = torch.sum(comp).item() / len(comp)  # item(): return the value of the tensor as a number
  
  pred_val = cls.predict(data_dict['X_val'])
  comp = torch.eq(pred_val, data_dict['y_val'])
  val_acc = torch.sum(comp).item() / len(comp)
  ############################################################################
  #                            END OF YOUR CODE                              #
  ############################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    scores = W.t().mv(X[i])
    scores = scores - max(scores)  # for numeric stability
    exp_s = torch.exp(scores) 
    norm_s = exp_s / torch.sum(exp_s)  # normalize
    loss += -torch.log(norm_s[y[i]])
    
    for j in range(num_classes):
      p = norm_s[j]  # probability for class j
      dW[:, j] += (p - (j == y[i]).double()) * X[i] 
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  scores = scores - max_scores  # for numeric stability
  p = torch.exp(scores) / torch.sum(torch.exp(scores), dim=1, keepdim=True)  # probabilities (normalize)
  
  idx0 = torch.arange(num_train)
  loss = torch.mean(-torch.log(p[idx0, y]))
  loss += reg * torch.sum(W * W)
  
  d_scores = p.clone()
  d_scores[idx0, y] -= 1  # for p - (j == y[i])
  dW = X.t().mm(d_scores)
  dW /= num_train
  dW += 2 * reg * W
  #############################################################################
...
def softmax_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [1.4e-1, 1.4e-2, 1.4e-3, 1.4e-4, 1.4e-5]
  regularization_strengths = [1.5e-1, 1.5e-2, 1.5e-3, 1.5e-4, 1.5e-5]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
