...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores = scores - torch.max(scores, dim=1, keepdim=True).values  # for numeric stability
    ...
    grads['W2'] = h1.t().mm(d_scores) / N + 2 * reg * W2
    grads['b2'] = torch.sum(d_scores, dim=0) / N
    
    d_h1 = d_scores.mm(W2.t())
    d_relu = h1 > 0
    d_scores1 = d_h1 * d_relu  # scores1 = X.mm(W1) + b1
    grads['W1'] = X.t().mm(d_scores1) / N + 2 * reg * W1
    grads['b1'] = torch.sum(d_scores1, dim=0) / N
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [1e0, 1e-1, 3e-1, 5e-1, 7e-1, 9e-1]
  hidden_sizes = [256, 512, 1024]
  regularization_strengths = [5e-4, 5e-5, 5e-6, 5e-7, 5e-8, 0]
    ...
    for lr in learning_rates:
      for rs in regularization_strengths:
        for lr_d in learning_rate_decays:
          net = TwoLayerNet(input_size, hs, num_classes, dtype=data_dict['X_train'].dtype, 
          ...
          # save all nets' results  
          net_name = 'hs' + str(hs) + '_lr' + str(lr) + '_rs' + str(rs) + '_lrde' + str(lr_d)
          print('%s validation accuracy: %.2f%%' % (net_name, val_acc))
          # results[net_name] = stats
          # results[net_name]['val_acc'] = val_acc
          
  # find top n nets based on val acc & print their acc curves        
  # top_nets_name = sorted(results, key=lambda i: results[i]['val_acc'], reverse=True)[:top_num]
  # print()
  # print('Top %d nets' % top_num)
  # for k in top_nets_name:
  #   print('%s validation accuracy: %.2f%%' % (k, results[k]['val_acc']))
  #   top_nets[k] = results[k]
  # plot_acc_curves(top_nets)
